...
describe("Get attendance by userId in month", () => {
    ...
    test("should handle attendances error when not provide month", async () => {
        const res = await request(server)
            .get(`/attendanceByMonth/${""}/${year}/${idExists}`)
            .set("Authorization", `Bearer ${loginRes.body.accessToken}`);
    ...
    test("should handle attendances error when not provide year", async () => {
        const res = await request(server)
            .get(`/attendanceByMonth/${monthExist}/${""}/${idExists}`)
            .set("Authorization", `Bearer ${loginRes.body.accessToken}`);
    ...
    test("should handle attendances error when not provide UserId", async () => {
        const res = await request(server)
            .get(`/attendanceByMonth/${monthExist}/${year}/${""}`)
            .set("Authorization", `Bearer ${loginRes.body.accessToken}`);
