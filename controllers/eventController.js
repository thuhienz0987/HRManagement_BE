...
const getEventsByDate = async (req, res) => {
    ...
    try {
      ...
      const events = await Event.find({
        isDeleted: false,
        date: { $gte: targetDate, $lte: new Date(targetDate.getTime() + 24 * 60 * 60 * 1000) },
      });
...
const postEvent = async (req, res) => {
 ...
 const eventsInSameTime = await Event.find({ dateTime: dateTime, isDeleted: false });
      ...
      for (const user of mandatoryUsers) {
          if (commonMandatoryUsers.includes(user.user.toString())) {
              throw new BadRequestError('Some mandatory participants attended another event during the same time frame');
          }
      }
...
const updateEvent = async (req, res) => {
  ...
  try {
      ...
      // Tìm tất cả sự kiện chưa bị xóa trong cùng khung giờ, trừ sự kiện hiện tại
      const eventsInSameTime = await Event.find({ dateTime: dateTime, isDeleted: false, _id: { $ne: id } });
...
export {getEvents,getEvent, getEventsByDate, getEventsByMonth, postEvent,updateEvent,deleteEvent}