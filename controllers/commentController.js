    ...
    try {
        ...
        if (!user) {
            throw new NotFoundError(
                `Reviewee with id ${revieweeId} does not exist`
            );
        }
...
const getCommentsByReviewerIdInMonth = async (req, res) => {
    try {
        ...
        if (user.isDeleted) {
            ...
            if (comments.length === 0) {
                throw new NotFoundError(
                    `Not found comments for user id ${reviewerId}`
                );
            }
...
const getLeaderNotCommentByDepartmentIdMonth = async (req, res) => {
    try {
        ...
        const leadersWithoutComments = leadersWithoutPassword.filter(
            (leader) => {
                return !comments.some(
                    (comment) =>
                        comment.revieweeId.toString() === leader._id.toString()
                );
            }
        );
...
const getEmployeeNotCommentByTeamIdMonth = async (req, res) => {
    try {
        ...
        const employeesWithoutComments = employeesWithoutPassword.filter(
            (user) => {
                return !comments.some(
                    (comment) =>
                        comment.revieweeId.toString() === user._id.toString()
                );
            }
        );
...
const getDepManagerNotCommentMonth = async (req, res) => {
    try {
        ...
        const managersWithoutComments = managersWithoutPassword.filter(
            (user) => {
                return !comments.some(
                    (comment) =>
                        comment.revieweeId.toString() === user._id.toString()
