    ...
    try {
        ...
        result.forEach((element) => {
            element.departmentPercent =
                element.departmentAttendances / totalAmount;
        });

        if (result.length === 0) {
            throw new NotFoundError(
                `No attendances found for ${month}/${year}`
            );
        }
        ...
        for (let day = 2; day <= daysInMonth + 1; day++) {
            ...
            for (const attendance of attendances) {
                if (attendance.checkInTime && attendance.checkOutTime) {
                    ...
                    const checkInTimeAfter1PM = new Date(
                        year,
                        month - 1,
                        day,
                        13,
            ...
            for (
                ...
                if (dayOfWeek !== 0 && dayOfWeek !== 6) {
                    ...
                    if (shouldCreateRecord) {
                        ...
                        const checkInTime = getRandomTime(
                            year,
                            month,
                            day,
                            6,
                            9
                        ); // Use the provided function to generate random check-in hour
                        const checkOutTime = getRandomTime(
                            year,
                            month,
                            day,
                            16,
                            20
                        ); // Use the provided function to generate random check-out hour
...
function getRandomTime(year, month, day, minHour, maxHour) {
    ...
    return new Date(
        ...
        0,
        0
    ); // Random time within the specified hour
}
